...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor_again() {
        Flux<Void> tasks = taskExecutor().concatMap(Flux::from);
    ...
    public void mail_box_switcher() {
        ...
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((signal, flux) -> {
                if (signal.hasValue()) {
                    Message msg = signal.get();

                    if (msg.metaData.equals("spam")) {
    ...
    public void acid_durability() {
        ...
        Flux<String> committedTasksIds = tasksToExecute()
                ...
                .flatMap((taskId) ->
                    this.commitTask(taskId).then(Mono.just(taskId)));

        //committedTasksIds.map(this::commitTask);
    ...
    public Mono<String> chooseSource() {
        return Mono.defer( () -> {
            if (this.sourceRef.get().equals("A")) {
                return sourceA();
            } else if (this.sourceRef.get().equals("B")) {
                return sourceB();
            }
    ...
    public void cleanup() {
        ...
        Flux<String> stream = Flux.usingWhen(
                ...
                Function.identity(),
                dummy -> StreamingConnection.closeConnection()
        );
