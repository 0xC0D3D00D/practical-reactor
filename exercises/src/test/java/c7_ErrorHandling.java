...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void under_the_rug() {
        Flux<String> messages = messageNode().onErrorResume((err) -> Flux.empty());
    ...
    public void error_reporter() {
        Flux<String> messages = messageNode().onErrorResume(
                e -> errorReportService(e)
                        .then(Mono.error(e))
        );
